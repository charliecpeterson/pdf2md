# Archived Project Plan: pdf2md (rework of docsmcp → PDF-to-markdown converter)

> Archived 2026-08-11. This file preserves the earlier decision log and deferred
> ideas; its checkboxes no longer track current work. Current product decisions are
> summarized in `README.md`, with candidate accuracy work in
...
> satisfied. See the 2026-06-21 Decision Log entry and the Phase 3 plan items.
>
...
## Decision Log
- **[2026-07-04] Figure-to-text workstream (chart data without the pixels) — COMPLETE**
  - **Context**: the goal was "a text-only LLM reads the md and rarely needs the crop."
    Vector charts already shipped CSV + a repro script; raster/scanned charts had only
    the opt-in low-confidence VLM estimate, and a hard scan (Ghia 1982: 14 crossing
    curves, two offset panels) would have gotten an invented table from it.
  - **Choice**: a model-free pixel pre-scan (`calibrate.py`) that gates AND anchors the
    VLM tier — tangled scans emit a visible withheld marker; calibrated ones ride their
    measured axis ranges into the prompt, and only an in-range, pixel-agreeing read can
    clear the emit floor. Vector tier extended (bars, multi-series scatter by marker
    style, caption/labels as repro-script comments), `--figure-svg` for the lossless
    vector form of non-chart figures, and a caption→table cross-reference (the printed
    table is often the authoritative text form a pixel trace can't beat). A live
    bake-off of local models (qwen3-vl 8b/32b, deepseek-ocr, glm-ocr) found none reads
    curve shape well enough to print — the confidence stack withheld every wrong read,
    which is the design working. Evals: `eval_raster.py` (gate/calibration, live-VLM
    half), `eval_digitize.py` extended (bars, 2-series scatter, both ≤0.1% y-error).
  - **Full narrative + model research**: `docs/figure-to-text.md`.
- **[2026-07-03] Whole-page vision transcription for scans (`--ocr-page-vlm`)**
  - **Context**: scanned prose stayed the weak class. Phase 4's `--ocr-vlm` re-OCR'd
    each scanned prose *block* (one model call per block, slow) but left equations and
    tables image-backed — so a scanned physics page came out as prose plus a wall of
    equation crops, not a source-of-truth transcript. And some PDFs whose embedded text
    is itself degraded OCR ("?3astman" for "Eastman") looked born-digital, so no OCR ran
    at all.
  - **Choice**: transcribe the whole page at once and replace every block on it.
    `--ocr-page-vlm` renders each scanned page, hands it to an OCR-tuned vision model,
    and swaps in one transcription block (+ the page's figure crops) — scanned equations
    come back as inline LaTeX, tables as GFM, so the page is text-sufficient, not a crop
    pile. `--force-ocr` (Docling `force_full_page_ocr` + `GlyphIndex(force_ocr=True)`)
    forces re-OCR when the embedded text layer is bad. Under page-VLM, Docling's own OCR
    is skipped (`do_ocr=False`) since every page is re-transcribed anyway, and scanned
    pages are enumerated straight from the PDF (`_pages_to_transcribe`) because with
    `do_ocr=False` there are no text blocks to enumerate from. A page that fails
    transcription emits a `#/page/N` marker (lossless), and the profile reports partial
    coverage honestly ("OCR by a vision model on 8/14 pages, engine OCR on the rest").
  - **Model**: **glm-ocr:q8_0 is the page-OCR model** (`--vlm-ocr-model glm-ocr:q8_0`).
    ~7s/page, exact, and transcribes scanned equations to LaTeX. deepseek-ocr:3b is
    faster but hallucinates; a general VLM (qwen3-vl) runs minutes/page. Distinct from the
    figure-*labels* path, where qwen3-vl:8b won (see 2026-07-02) — page transcription and
    figure-label reading want different models, hence the separate `vlm_ocr_model` lever.
  - **Outcome**: this largely **obviates the deferred MinerU / second-backend** for the
    scanned-equation gap — MinerU's one decisive edge in the bake-off was scanned-equation
    completeness, which glm-ocr page transcription now delivers without a second engine.
    Validated: NASA report exact; Slater (511pp scan) transcribed every page, Planck's-law
    and friends as correct LaTeX, 69 figure crops only, lossless, ~6.7min cached after the
    skip-OCR optimization. Also added a **text-sufficiency** grade (`profile._sufficiency`):
    per document, how many elements are usable from the markdown alone vs. pixel-authoritative
    (crop is the record), surfaced in profile.json and README.md.
  - **Revisit if**: pages fail transcription often enough to matter — the next lever is a
    RapidOCR fallback for failed pages rather than a bare marker. Also if a non-Ollama
    endpoint needs different escaping than the `_decode_escaped_lines` fix.

- **[2026-07-02] `--figure-labels` hardening + OCR-model bake-off**
  - **Context**: `--figure-labels` (read the printed text off a figure crop) shipped but
    was validated only by eyeballing. Running it across image/GRASP/slater exposed a chain
    of vision-model failure modes: digit slips on peak values, describing-sentence
    narration leaking in, a value hallucinated inside a sentence, majority-voting discarding
    the vote that read the most, LaTeX-delimiter duplicate labels, and (on the 511-page
    slater scan) counting loops that run to the token cap.
  - **Choice**: model-agnostic robustness in the read path — numeric consensus across votes,
    per-read narration stripping, union-merge of votes (keep what any vote saw; flag only a
    genuine slip, not "one read more"), delimiter-normalized dedup, doc-level read caching,
    and a **configurable** generation cap (`Config.figure_labels_max_tokens`, default None).
  - **Bake-off** (new `scripts/eval_figure_labels.py` + hand-labelled
    `tests/figure_labels_labels.json`, 3 figures): **qwen3-vl:8b numeric accuracy 0.889 vs
    glm-ocr:bf16 0.630 vs deepseek-ocr:3b 0.370.** glm-ocr is non-deterministic (one figure
    scored 1/16 then 16/16 on identical input); deepseek-ocr hallucinates (invented a
    molecule/axis story). qwen3-vl:8b is clean, complete, and loop-free.
  - **Outcome**: the existing default (`vlm_model=qwen3-vl:8b`, `vlm_ocr_model=None`) is
    already the right model; **do not override to glm-ocr**. A hardcoded 1024-token labels
    cap was found to break reasoning models (qwen returns empty when the cap starves its
    thinking) and was made configurable, default off. Partially answers the deferred
    "better OCR / second backend" question for the figure/label path: the lever was model
    choice, not a new backend.
  - **Revisit if**: figures where votes read different *regions* prove common (union-merge
    keeps both but can't reconcile a slip vs two real peaks without spatial info), or a
    counting-loop model must be supported (extend `collapse_repeats`, currently left
    conservative for the shared OCR-prose path).
...
- [~] Second engine backend (MinerU / PaddleOCR-VL) behind the seam — bake-off RAN
      ...
      Its only decisive edge was scanned-equation completeness, now **largely closed by
      glm-ocr whole-page transcription** (`--ocr-page-vlm`, 2026-07-03) — so the MinerU
      adapter is likely obviated unless a non-equation gap surfaces. See Decision Log.
