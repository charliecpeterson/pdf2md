...
def test_model_routing_by_kind():
    ...
    assert d.model_for("page") == "ocr"          # scanned-page text -> OCR model
    assert d.model_for("equation") == "vlm"      # equations transcribe cleaner on the VLM
    assert d.model_for("digitize") == "ocr"      # reasoning VLMs think past the budget on JSON
...
def test_describer_built_for_page_ocr_only():
    pytest.importorskip("openai")
    # --ocr-page-vlm alone (no --describe) must still build a describer.
    assert get_describer(replace(Config(), ocr_page_vlm=True)) is not None
...
def test_describe_crops_routes_by_kind(tmp_path):
    from pdf2md.figure_passes import _describe_crops
...
def test_interrupted_descriptions_reuse_completed_regions(tmp_path):
    from pdf2md.figure_passes import _describe_crops
    from pdf2md.schema import BBox, FigureRef
    ...
    figures = [
        FigureRef(f"#/{name}", 1, BBox(0, 1, 1, 0), asset_path=f"assets/{name}")
        for name in ("first.png", "second.png")
    ]
    interrupted = Interrupting()
    with pytest.raises(RuntimeError, match="interrupted"):
        _describe_crops(figures, [], interrupted, tmp_path)
    ...
    class Resumed:
        ...
        def describe(self, image_path, kind, context=""):
            self.calls.append(image_path.name)
            return f"{image_path.stem} description"
    ...
    resumed = Resumed()
    _describe_crops(resumed_figures, [], resumed, tmp_path)

    assert resumed.calls == ["second.png"]
    assert resumed_figures[0].description == "first description"
    assert resumed_figures[1].description == "second description"
