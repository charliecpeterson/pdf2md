...
def test_check_gates_only_on_hard_invariants():
    base = {"d.pdf": {"accounted_for": True, "dropped": 0, "ligature_residual": 0,
                      "unbalanced_eq": 0, "illegible": 0, "illegible_table_rows": 0,
                      "eq_image_backed": 5}}
    # image-backing dropping is drift, not a regression; accounting loss and a risen
    # dropped count are.
    cur = {"d.pdf": {"accounted_for": False, "dropped": 2, "ligature_residual": 0,
                     "unbalanced_eq": 0, "illegible": 0, "illegible_table_rows": 0,
                     "eq_image_backed": 1}}
    regressions = qa._check(cur, base)
    assert any("accounted_for" in r for r in regressions)
...
def test_check_gates_on_illegible_regression():
    ...
    base = {"d.pdf": {"accounted_for": True, "dropped": 0, "ligature_residual": 0,
                      "unbalanced_eq": 0, "illegible": 0, "illegible_table_rows": 0}}
    cur = {"d.pdf": {"accounted_for": True, "dropped": 0, "ligature_residual": 0,
...
def test_check_gates_on_missing_baseline_document():
    baseline = {"missing.pdf": {"accounted_for": True, "dropped": 0}}
    assert qa._check({}, baseline) == ["missing.pdf: missing output"]
