...
def test_auto_engine_picks_docling_for_a_born_digital_document(monkeypatch):
    ...
    monkeypatch.setattr(pipeline, "_scanned_share", lambda _p: 0.0)

    assert pipeline._auto_engine(Config(engine="auto"), Path("x.pdf")) == "docling"
...
def test_auto_engine_picks_mineru_for_a_scan_when_it_is_installed(monkeypatch):
    ...
    assert pipeline._auto_engine(Config(engine="auto"), Path("x.pdf")) == "mineru"
