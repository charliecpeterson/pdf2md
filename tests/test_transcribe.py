...
def test_surya_transcriber_adds_blank_edge_context(tmp_path):
    from PIL import Image

    image_path = tmp_path / "equation.png"
    Image.new("RGB", (120, 40), "black").save(image_path)
    seen = []
    transcriber = object.__new__(SuryaTranscriber)
    transcriber._run = lambda image: seen.append(image.copy()) or "<math>x</math>"

    assert transcriber.transcribe(image_path) == "x"
    assert seen[0].size == (280, 200)
    assert seen[0].getpixel((0, 0)) == (255, 255, 255)
    assert seen[0].getpixel((80, 80)) == (0, 0, 0)
