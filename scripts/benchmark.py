...
def main() -> None:
    ...
    rows: list[dict] = []
    hdr = (f"{'DOC':40s} {'PAGES':>5} {'SEC':>8} {'PG/S':>6} {'BLK':>5} "
           f"{'CROP':>4} {'REV':>4}  ACCT COMP")
    print(hdr)
    print("-" * len(hdr))
    for pdf in _pdfs(args.paths):
        ...
        if r.failed:
            ...
            continue
        c = r.coverage
        review_markers = len(c.flags) if c else 0
        pps = r.page_count / dt if dt else 0.0
        print(
            f"{pdf.name[:40]:40s} {r.page_count:5d} {dt:8.1f} {pps:6.2f} "
            f"{c.total_blocks if c else 0:5d} {c.cropped if c else 0:4d} {review_markers:4d}  "
            f"{bool(c and c.accounted_for)!s:4s} {bool(c and c.complete)}"
        )
        rows.append(
            {
                "doc": pdf.name,
                ...
                "accounted_for": bool(c and c.accounted_for),
                "complete": bool(c and c.complete),
                "needs_review": bool(c and c.needs_review),
            }
        )
