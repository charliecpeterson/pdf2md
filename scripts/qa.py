...
scanned-page detection, and block accounting. `--check` compares against a committed
baseline and exits non-zero if a *hard invariant* regressed (accounting lost,
...
# Counts that must never rise (and accounted_for must stay True). Everything else is
...
# `orphaned_crops` guards a subtler loss the raw `dropped` count misses: a block marked
# dropped that still has a rendered crop — content discarded despite an image sitting in
# assets/ (the --no-formula equation bug). It must stay 0; a dropped block with a crop
# should have emitted the crop instead.
_INVARIANTS = ("dropped", "ligature_residual", "unbalanced_eq", "illegible",
               "illegible_table_rows", "orphaned_crops")
...
def _signals(version_dir: Path) -> dict | None:
    ...
    return {
        ...
        "complete": status("flagged") == 0 and status("dropped") == 0 and total == buckets,
        "dropped": status("dropped"),
        "orphaned_crops": sum(
            1 for b in blocks
            if b.get("coverage_status") == "dropped" and (b.get("extra") or {}).get("crop_path")
        ),
        "eq_total": len(eqs),
...
def _print_table(sigs: dict[str, dict]) -> None:
    hdr = (f"{'DOC':28s} {'PG':>3} {'BLK':>4} {'ACCT':>4} {'COMP':>4} {'DROP':>4} {'ORPH':>4} "
