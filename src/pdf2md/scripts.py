...
def _scriptsplit_text(chars: list[Char]) -> str:
    """Space-separate scripted glyph groups from their baseline neighbours (see
    `PageChars.text_scriptsplit`). Module-level so it's testable without a page."""
    out: list[str] = []
    prev: str | None = None
    for line in _lines(chars):
        for ch, flag in _score_line(line):
            if not ch.strip():
                out.append(ch)
                prev = None
                continue
            if flag != prev and out and out[-1].strip():
                out.append(" ")
            out.append(ch)
            prev = flag
        out.append("\n")
...
class PageChars:
    ...
    def region_chars(self, bbox) -> list[Char]:
        """Char boxes whose centers fall inside `bbox` — geometry-level access for
        rebuilders that need positions, not just `text_region`'s flat string."""
        return self._region(bbox)

    def text(self) -> str:
        """The page's whole embedded reading, flat. The source side of the
        document-level numeric-conservation check, which must see everything the
        layer holds — not just the regions engines turned into blocks."""
        return "".join(c[0] for c in self._chars)

    def text_scriptsplit(self) -> str:
        """`text()` with a space inserted at every super/subscript group boundary.
        Draw order glues exponents onto their base (`1019` for 10^19); typeset
        output renders them apart, so a numeric comparison against rendered
        markdown must tokenize the layer the same way or every exponent reads as
        a phantom loss."""
