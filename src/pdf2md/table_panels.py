"""Repeated side-by-side panels, stitched back into one long-form dataset.

A table typeset as three panels across the page is three tables, and reading it
as one wide grid puts a nickel row beside a rare-earth row. `tables.panel_tables`
emits them separately for a reader; this writes the machine-readable form —
one record per printed row, with the panel it came from and the checks that ran
over it.

Nothing here promotes a value. The review signals (numeric outliers, column
consistency) say where to look, and the raw grid stays beside them.
"""
