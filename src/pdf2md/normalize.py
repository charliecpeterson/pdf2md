...
# A sentence stop between a lowercase and an uppercase letter ('marker.The'). The
# lowercase-before guard protects acronyms ('N.A.C.A.') and the digit-neither guard
# protects decimals/versions ('3.5', 'v2.0'), which never sit between lower and upper.
_SPACE_AFTER_STOP = re.compile(r"(?<=[a-z])([.!?])(?=[A-Z])")


def space_after_punct(text: str) -> str:
    """Insert the space RapidOCR drops after punctuation inside a scanned line: after a comma or
    semicolon before a letter ('ramp,toward' -> 'ramp, toward'), and after a sentence stop that
    sits between a lowercase and an uppercase letter ('marker.The' -> 'marker. The'). The guards
    leave a thousands/decimal comma ('3,000'), an acronym ('N.A.C.A.'), and a version ('v2.0')
    alone. Language-agnostic — a missing space after a comma is wrong in any language, so this
    runs even when word-split is off."""
    return _SPACE_AFTER_STOP.sub(r"\1 ", _SPACE_AFTER_COMMA.sub(r"\1 ", text))
...
def resegment_words(text: str) -> str:
    """Re-insert the spaces RapidOCR drops inside a scanned line ('wherethefirst' ->
    'where the first'). Only alphabetic runs are touched, so numbers and tokens like 'swim+8'
    survive; wordninja keeps real words and proper nouns whole (Sonic, Reynolds, configuration
    all stay), so an already-spaced line is a no-op. English word frequencies, so the caller
    gates this for non-English scans. Returns the text unchanged if wordninja isn't importable."""
    try:
        import wordninja
    except ImportError:
        return text
